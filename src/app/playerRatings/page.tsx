...
export default function Component() {
  ...
  useEffect(() => {
    ...
    fetch(
      'http://localhost:8081/academico/listar?page=0&size=10&sort=curso,desc',
      {
        headers: {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token')
    fetch(
      'http://localhost:8081/campeonatos/11/inscritos?page=0&size=10&sort=dataCriacao,desc',
      {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const avaliarJogador = (id: number, novaAvaliacao: number) => {
    console.log(`Jogador avaliado: ID=${id}, Avaliação=${novaAvaliacao}`)
            ...
            >
              {jogadoresFiltrados.map((jogador, index) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <h2 className="text-lg font-semibold dark:text-gray-500">
                        {jogador.nome}
                      </h2>
                    ...
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((estrela) => (
                        <Button
                          key={`${jogador.id}-${estrela}`} // Updated key
                          variant="ghost"
                          size="sm"
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="mt-2 space-y-1">
                        ...
                        <p className="text-sm">
                          Criador: {campeonato.usernameCriador}
                        </p>
                        <p className="text-sm">
                          Status: {campeonato.situacaoCampeonato}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-sm mt-2">
                        ...
                        até {new Date(campeonato.dataFim).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                </motion.div>
